    ...
    def get_actor_dialogue_response(
        ...
        def generate_response(
            ...
            while True:
                ...
                prompt = self.guidance(full_prompt, silent=self.silent, logit_bias={ 198: -10 })
                try:
                    result = prompt(
                        name=name,
        ...
        if examples != "!<NoCorpus>!":
            if examples != "":
                hint += f"When facing reporters, you may refer to the content and tone of the following dialogue examples: \n{examples}Note the differences between the questions and avoid simply copying examples.\n"
            else:
                pass
