...
from generativeActor import GenerativeActor
from generativeDirector import GenerativeDirector
...
class GenerativeStage:
    ...
    def parse_player_action(self, player: Player):
        if player.action["action"] == "move":
            target_act_name = player.action["content"]
            ...
            current_director.add_dialogue_log({"role": player.name, "content": player.action["content"]})
    ...
    def save_stage_log(self, path="server/logs", name="dialogue_log"):
        LOG_NAME = name + ".json"
        logs = {actor_name: self.actors[actor_name].dialogue_history.dialogue_history for actor_name in self.actors}
        with open(os.path.join(path, LOG_NAME), "w", encoding="utf-8") as fw:
